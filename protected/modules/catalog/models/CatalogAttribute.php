 ...
 */
class CatalogAttribute extends BasicModel
{
    const TYPE_VALUE = 1;
    const TYPE_FIELD = 2;
    const TYPE_LIST = 3;
    ...
    protected function afterSave()
    {
        ...
        //Если атрибут типа флаг, то создаем для него значение с именем атрибута
        if ($this->id_attribute_kind == self::TYPE_FLAG) {
            if ($this->values) {
                $val = CatalogAttributeValue::model()->find("id_attribute=$this->id");
            } else {
                $val = new CatalogAttributeValue;
                $val->id_attribute = $this->id;
            }
            $val->value = $this->title;
            $val->save();
        }
    }
