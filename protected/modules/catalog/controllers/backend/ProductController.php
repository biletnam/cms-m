...
class ProductController extends BackEndController
{
	...
	public function actionCreate($id_category)
	{
        ...
        $imageDataProvider=new CActiveDataProvider('CatalogImage', array(
            'criteria'=>$criteria,
            'pagination'=>false,
        ));
    ...
    public function actionDuplicate($id){
        ...
        if (isset($_POST['DuplicateForm'])) {
            ...
            if ($parameters->validate()) {
                ...
                if(!$parameters->image_duplicate){$new_model->image='';}
                ...
                if($success){
                    ...
                    if($parameters->image_duplicate && $model->image){
                        $imagefile=Yii::app()->file->set($model->folder . '/' . $model->image);
                        $imagefile_medium=Yii::app()->file->set($model->folder . '/medium/' . $model->image);
                        $imagefile_small=Yii::app()->file->set($model->folder . '/small/' . $model->image);

                        $new_model->image=md5(time().$imagefile->filename).'.'.$imagefile->extension;

                        $new_imagefile=$imagefile->copy($new_model->image);
                        $new_imagefile_medium=$imagefile_medium->copy($new_model->image);
                        $new_imagefile_small=$imagefile_small->copy($new_model->image);
