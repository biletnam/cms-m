...
class Yml
{
	...
	function prepare_field($s)
	{
		...
		if ($this->from_charset!='windows-1251') $s = mb_convert_encoding($s, 'windows-1251', $this->from_charset); 
	...
	# @id - id рубрики
	# @id_category - id родительской рубрики, если нет, то -1
	# @name - название рубрики
	function add_category($name, $id, $id_category = -1)
	{
		if ((int)$id<1||trim($name)=='') return false;
		if ((int)$id_category>0)
			$this->categories[] = array('id'=>(int)$id, 'parentId'=>(int)$id_category, 'name'=>$this->prepare_field($name));
		else
	...
	{
		$allowed = array('url', 'price', 'currencyId', 'categoryId', 'picture','store','pickup', 'delivery', 'local_delivery_cost', 'name', 'vendor', 'vendorCode', 'description', 'sales_notes','manufacturer_warranty', 'country_of_origin', 'downloadable');
	...
	function get_xml_shop()
	{
		$s = '<shop>' . "\r\n";
			...
			$s .= '<local_delivery_cost>'.$this->local_delivery_cost.'</local_delivery_cost>' . "\r\n";
